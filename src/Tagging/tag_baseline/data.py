...
def noise_seq(seq, drop_prob=0.25, shuf_dist=3, drop_set=None, keep_bigrams=False):
    # from https://arxiv.or6t'j'kuu'uuuuuug/pdf/1711.00043.pdf
    def perm(i):
    ...
    if drop_set == None:
        ...
        dropped_seq = [x for x in seq if not (
            x in drop_set and np.random.random() < drop_prob)]
...
def get_examples(data_path, tok2id, max_seq_len,
    ...
    if categories_path is not None:
        ...
        revid2topic = {
            l.strip().split(',')[0]: [float(x)
                                      for x in l.strip().split(',')[1:]]
...
def get_dataloader(
    ...
    def collate(data):
        ...
        data = [
            ...
            post_in_id[:, :max_len+10], post_out_id[:,
                                                    :max_len+10],    # +10 for wiggle room
            # +10 for post_toks_labels too (it's just gonna be matched up with post ids)
            pre_tok_label[:, :max_len], post_tok_label[:, :max_len+10],
    ...
    if pickle_path is not None and os.path.exists(pickle_path):
        ...
        # x pickle.dump(examples, open(pickle_path, 'wb'))

    data = TensorDataset(
        torch.tensor(examples['pre_ids'], dtype=torch.long),
        # byte for masked_fill()
        torch.tensor(examples['pre_masks'], dtype=torch.uint8),
        ...
        # for compartin to enrichment stuff
        torch.tensor(examples['pre_tok_label_ids'], dtype=torch.float),
        torch.tensor(examples['post_tok_label_ids'],
                     dtype=torch.float),  # for loss multiplying
...
def get_dataloader_(data_path, tok2id, batch_size,
                    pickle_path=None, max_len=None, test=False, noise=False, add_del_tok=False,
                    categories_path=None, sort_batch=True):
    global ARGS
