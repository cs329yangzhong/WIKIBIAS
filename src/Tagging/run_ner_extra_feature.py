...
class JointTag_withfeature(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None, attention_mask_label=None, class_label=None, pos_ids=None, rel_ids=None, w1=1, w2=2, w3=0.5, batch_size_=8, CUDA=(torch.cuda.device_count() > 0)):
        ...
        if self.config.output_hidden_states:
            ...
            sequence_oiutput = hidden_states[-4]
...
class JointBiasProcessor(DataProcessor):
    ...
    # def get_train_clean_examples(self, data_dir):
    #     """See base class."""""
    #     return self._create_examples(
    #         self._read_joint_tsv(os.path.join(data_dir, "tagging_train_source_clean_02022021.tsv")), "train_cleanannotate"
    #                      )

    def get_train_noisy_auto_examples_0510(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_joint_tsv(os.path.join(
                data_dir, "filtered_noisy_train.tsv")), "train_noisyannotate_0510"
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "train_source.tsv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "dev_source.tsv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""

        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "test_source.tsv")), "test")

    def get_train_examples_neg(self, data_dir):
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "noisy_train_neg")), "train_neg")

     def get_train_examples_clean(self, data_dir):

        return self._create_examples(
                             self._read_joint_tsv(os.path.join(data_dir, "train_source.tsv")), "train_neg_clean"
                )
    def get_train_examples_clean_neg(self, data_dir):

        return self._create_examples(
                             self._read_joint_tsv(os.path.join(data_dir, "train_target.tsv")), "train_neg_clean"
                )
    def get_dev_examples_neg(self, data_dir):
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "dev_target.tsv")), "dev")

    def get_test_examples_neg(self, data_dir):
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "test_target.tsv")), "test")
    ...
    def _create_examples(self,lines,set_type):
        ...
        for i,(sentence, label, class_label) in enumerate(lines):
            ...
            text_b = None
            label = label
            examples.append(InputExample(guid=guid,text_a=text_a,text_b=text_b,label=label, class_label=class_label))
        return examples
...
def main():
    ...
    processors = {"ner":NerProcessor, "tag":BiasProcessor, 'joint':JointBiasProcessor}
    ...
    if args.do_train:
        train_examples = processor.get_train_clean_examples(args.data_dir)
    ...
    # Prepare model
    config = BertConfig.from_pretrained(args.bert_model, num_labels=num_labels, finetuning_task=args.task_name, output_hidden_states=False)
    ...
    if args.do_joint != True:
        ...
        model = JointTag_withfeature(args=args, config=config, token_num_labels=num_labels,tok2id=tokenizer.vocab)
        if args.pretrained_tagging_model != "":
            load_model = JointTag_withfeature(args=args, config=config, token_num_labels=num_labels)
            load_model.load_state_dict(torch.load(os.path.join("out_tag_cased_0.4_1_0.6_new", "model.pt")))
    ...
    if args.do_train:
        if args.do_joint:
            ...
            # Noisy
            train_examples = processor.get_train_noisy_auto_examples_0510(args.data_dir)
            train_features = convert_examples_to_features_with_addition(
            train_examples, label_list, args.max_seq_length, tokenizer, name_file='train_noisyannotate_auto_0510')
            ...
            #### Clean Data, uncommit if you want to train the model only on the manually anntoated data.
    
            # train_examples = processor.get_train_examples_clean(args.data_dir)
            # train_features = convert_examples_to_features_with_addition(
            # train_examples, label_list, args.max_seq_length, tokenizer, name_file='train_clean')
            # train_examples = processor.get_train_examples_clean_neg(args.data_dir)
            # train_neg_features = convert_examples_to_features_with_addition(
            # train_neg_example, label_list, args.max_seq_length, tokenizer, name_file='train_clean_neg')
        else:
        ...
        train_dataloader = create_loader(train_features, args, args.train_batch_size, test=False)
        train_neg_dataloader = create_loader(train_neg_features, args, args.train_batch_size, test=False)
        ...
        if args.do_joint:
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                    ...
                    loss = model(torch.cat([input_ids, input_ids_2], dim=0),
                                 torch.cat([segment_ids, segment_ids_2], dim=0),
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                    ...
                    loss = model(input_ids, segment_ids, input_mask, label_ids, valid_ids, l_mask, cls_id)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.do_joint:
            ...
            eval_features = convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer, name_file='dev_neg')
        ...
        eval_dataloader = create_loader(eval_features, args,args.eval_batch_size, test=True)
        eval_data_neg_loader = create_loader(eval_neg_features, args,args.eval_batch_size, test=True)
        ...
        if args.do_joint:
            ...
            for input_ids, input_mask, segment_ids, label_ids,valid_ids,l_mask, class_id, pos_id, dep_id in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    cls_logits, logits = model(torch.cat([input_ids, input_ids_2], dim=0),
                                    ...
                                    valid_ids=torch.cat([valid_ids, valid_ids_2], dim=0),
                                    attention_mask_label=torch.cat([l_mask, l_mask_2], dim=0),
                ...
                cls_label_ids = torch.cat([class_id, cls_id_2], dim=0).to('cpu').numpy()
                print(cls_logits)
                print(cls_label_ids)
                # raise EOFError
            ...
            accuracy= sum(1 for x,y in zip(y_cls_true,y_cls_pred) if x == y) / len(y_cls_pred)
            print("Accuracy is ", accuracy)
            print(classify_report)
            import pickle
            file1 = pickle.dump([y_cls_true, y_cls_pred], open("%s_out_%s_classification_result.p"%(args.eval_on, args.output_dir), "wb"))
            write_file = open("%s_out_%s_task_ratio%s.txt"%(args.eval_on, args.output_dir, args.task_ratio), "w", encoding="utf-8")
            data_tuples = read_special_format(os.path.join(args.data_dir, "tag_%s.tsv"%args.eval_on))
            ...
            logger.info("\n%s", report)
            output_eval_file = os.path.join(args.output_dir, "%s_eval_results.txt"%args.eval_on)
            ...
            write_file = open("%s_out_%s.txt"%(args.eval_on, args.output_dir), "w", encoding="utf-8")
            data_tuples = read_special_format(os.path.join(args.data_dir, "tag_%s.tsv"%args.eval_on))
            for id_pred, line in enumerate(y_pred):
