...
def countChunks_(args, inputFile):
    """
    ...
    for line in fileIterator:
        ...
        if inCorrect:
            endOfGuessed = endOfChunk(
                lastCorrect, correct, lastCorrectType, correctType)
            endOfCorrect = endOfChunk(
                lastGuessed, guessed, lastGuessedType, guessedType)
        ...
        startOfGuessed = startOfChunk(
            lastGuessed, guessed, lastGuessedType, guessedType)
        startOfCorrect = startOfChunk(
            lastCorrect, correct, lastCorrectType, correctType)
...
def evaluate_(correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter, latex=False, to_tsv=False, tsv_file_name="perf.tsv"):
    # sum counts
    ...
    if to_tsv:
        ...
        for i in sortedTypes:
            precision, recall, FB1 = calcMetrics(
                correctChunk[i], foundGuessed[i], foundCorrect[i])
            opline = i.replace("_", " ") + "\t" + str(precision) + \
                "\t" + str(recall) + "\t" + str(FB1)+"\n"
            fout_tsv.write(opline)
        ...
        precision, recall, FB1 = calcMetrics(
            correctChunkSum, foundGuessedSum, foundCorrectSum)
        opline = "overall" + "\t" + \
            str(precision) + "\t" + str(recall) + "\t" + str(FB1)+"\n"
        fout_tsv.write(opline)
        fout_tsv.close()
...
# JT : 2018-08
def evaluate_conll_file_partial(inputFile="conll_output.txt", to_tsv=False, tsv_file_name="perf.tsv", latex=False, raw=False, delimiter=None, oTag="O"):
    #args = parse_args()
    args = {}
    args["raw"] = raw
    args["latex"] = latex
    ...
    correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks_(
        args, inputFile)

    # compute metrics and print
    eval_result = evaluate_(correctChunk, foundGuessed, foundCorrect, correctTags,
                            tokenCounter, latex=latex, to_tsv=to_tsv, tsv_file_name=tsv_file_name)
    return eval_result
