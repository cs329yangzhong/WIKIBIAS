...
class JointTag(nn.Module):
    def __init__(self, args, config, cls_num_labels=2, token_num_labels=None):
        super().__init__()
        self.config = config
        self.args = args
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None, attention_mask_label=None, class_label=None, w1=1, w2=2, w3=0.5, batch_size_=8):
        self.batch_size = batch_size_
        output = self.bert(input_ids, token_type_ids,
                           attention_mask, head_mask=None)

        # print(input_ids.shape)
        # print("ddd" , len(output))
        ...
        if labels is not None:
            ...
            tag_logits = self.token_classifier(
                sequence_output[:self.batch_size])
            cls_logits = self.cls_classifier(cls_rep)
...
class JointBiasProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "tagging_train_source_0510.tsv")), "train")

    def get_train_augment_examples(self, data_dir):
        return self._create_examples(
            self._read_joint_tsv(os.path.join(
                data_dir, "noisy_train_augment_plus.tsv")), "train"
        )

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "tag_dev.tsv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""

        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "tag_test.tsv")), "test")

    def get_train_examples_neg(self, data_dir):
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "../noisy_train_neg.tsv")), "train")

    def get_dev_examples_neg(self, data_dir):
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "tag_dev_neg.tsv")), "dev")

    def get_test_examples_neg(self, data_dir):
        return self._create_examples(
            self._read_joint_tsv(os.path.join(data_dir, "tag_test_neg.tsv")), "test")
...
def create_loader(features, args):
    ...
    if args.do_joint:
        ...
        print(all_cls_label_ids)
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_label_ids, all_valid_ids, all_lmask_ids, all_cls_label_ids)
    else:
        all_cls_label_ids = torch.tensor(
...
def main():
    ...
    if args.do_joint != True:
        ...
        model = JointTag(args=args, config=config, token_num_labels=num_labels)
        if args.pretrained_tagging_model != "":
            load_model = Tag.from_pretrained("out_tag_cased")
            model.load_pretrain(load_model.bert, load_model.classifier)
            # raise EOFError
    ...
    if args.do_train:
        ...
        if args.do_joint:
            ...
            train_features = convert_examples_to_features_joint(
                train_examples, label_list, args.max_seq_length, tokenizer)
            train_neg_features = convert_examples_to_features_joint(
                train_neg_example, label_list, args.max_seq_length, tokenizer)
        else:
            train_features = convert_examples_to_features(
                train_examples, label_list, args.max_seq_length, tokenizer)
            train_neg_features = convert_examples_to_features(
                train_neg_examples, label_list, args.max_seq_length, tokenizer)
        ...
        train_data = create_loader(train_features, args)
        train_data_neg = create_loader(train_neg_features, args)
        ...
        if args.local_rank == -1:
            ...
            train_neg_sampler = DistributedSampler(train_data_neg)

        train_dataloader = DataLoader(
            train_data, sampler=train_sampler, batch_size=args.train_batch_size)
        train_neg_dataloader = DataLoader(
            train_data_neg, sampler=train_neg_sampler, batch_size=args.train_batch_size)
        ...
        if args.do_joint:
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                    ...
                    input_ids_2, input_mask_2, segment_ids_2, label_ids_2, valid_ids_2, l_mask_2, cls_id_2 = neg_batch
                    print(cls_id_2)
                    loss = model(torch.cat([input_ids, input_ids_2], dim=0),
                                 ...
                                 torch.cat([cls_id, cls_id_2], dim=0), w1=w1, w2=w2, w3=w3)
        ...
        if args.do_joint:
            model = JointTag(args=args, config=config,
                             token_num_labels=num_labels)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.do_joint:
            eval_features = convert_examples_to_features_joint(
                eval_examples, label_list, args.max_seq_length, tokenizer)
            eval_neg_features = convert_examples_to_features_joint(
        ...
        eval_data = create_loader(eval_features, args)
        eval_data_neg = create_loader(eval_neg_features, args)

        if args.do_joint:
            # Run prediction for full data
            eval_sampler = SequentialSampler(eval_data)
            eval_neg_sampler = SequentialSampler(eval_data_neg)
            eval_dataloader = DataLoader(
                eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
            eval_neg_dataloader = DataLoader(
                eval_data_neg, sampler=eval_neg_sampler, batch_size=args.eval_batch_size)

            eval_neg_iterator = iter(eval_neg_dataloader)
            model.eval()
...
if __name__ == "__main__":
    ...
    # lower cased.
    # python run_ner.py --data_dir=data/biased_tag --bert_model=bert-base-uncased --task_name=tag --output_dir=out_tag_uncased --max_seq_length=128 --do_train --num_train_epochs 3 --do_eval --warmup_proportion=0.1
    # python run_ner.py --data_dir=data/biased_tag --bert_model=bert-base-uncased --task_name=tag --output_dir=out_tag_uncased --max_seq_length=128 --num_train_epochs 5 --do_eval --warmup_proportio=0.1 --eval_on test --do_lower_case

    # joint.
    # python run_ner.py --data_dir=data/biased_tag --bert_model=bert-base-cased --task_name=joint --output_dir=out_tag_joint_cased_x --max_seq_length=256 --do_train --num_train_epochs 3 --do_eval --warmup_proportion=0.1 --task_ratio 0.3
    # python run_ner.py --data_dir=data/biased_tag --bert_model=bert-base-uncased --task_name=joint --output_dir=out_tag_joint_uncased --max_seq_length=256 --num_train_epochs 5 --do_eval --warmup_proportio=0.1 --eval_on test --do_lower_case

    # cased.
    # python run_ner.py --data_dir=data/biased_tag --bert_model=bert-base-cased --task_name=tag --output_dir=out_tag_cased --max_seq_length=128 --do_train --num_train_epochs 3 --do_eval --warmup_proportion=0.1
    # python run_ner.py --data_dir=data/biased_tag --bert_model=bert-base-cased --task_name=tag --output_dir=out_tag_cased --max_seq_length=128 --num_train_epochs 5 --do_eval --warmup_proportio=0.1 --eval_on test
